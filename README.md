...
| 源正文 / 中文产出 | 源正文 591,975 字符（盘点口径；分段口径 633,993）→ 译文 **270,210** 字符，其中汉字 151,643 | [`quality/coverage.md`](quality/coverage.md) |
...
> **字符口径别混用**：591,975 是 01 盘点的「可翻译正文」字符数；03 分段后提交翻译的单元共 633,993 字符（含标题行与列表标记）；当前译文文本 270,210 字符，其中汉字 151,643。三个数字来源不同，不要互相替换引用。
...
├─ tools/                     # 流水线脚本 01→13 + glossary.json（机器可读术语表）
...
| 6 校验 | `tools/06`–`13` | 术语一致性 / Han 占比 / 占位符残留 / 覆盖度 / 指标重算 / 排版修正 / 报告 | `quality/*.md` |
...
python tools/06_terminology_check.py   # 之后依次 07→13
```
...
| 3 | 翻译流水线**可复跑**：换一门课同样流程能再产出 | 13 个脚本全链路 + `glossary.json`；换源只需替换清单与术语表两个数据文件 | [§5](#五流水线怎么复跑含换源复用)、[`tools/`](tools) |
...
| 维度 | 满分 | 交付了什么 | 证据 |
|---|---|---|---|
| contentAccuracy | 25 | 30 篇译文共 270,210 字符（其中汉字 151,643）；术语 0 违规；逐篇 Han 占比抽检 | [`quality/抽检报告.md`](quality/抽检报告.md) |
| pipelineAutomation | 20 | `01→13` 全链路脚本、断点续传、换源只改数据 | [`tools/`](tools)、§5 |
| artifactCompleteness | 15 | README、来源清单、术语表、AI 日志、七维 AAR、3 篇拿来说明 | [`dist/`](dist) |
| aiUsage | 20 | AI 日志记录真实 prompt 与工具链；拿来说明含「原文 / prompt / 产出 / 对比」四段 | [`dist/AI日志.md`](dist/AI日志.md)、[`dist/拿来说明-01-为什么把翻译做成可复跑流水线.md`](dist/拿来说明-01-为什么把翻译做成可复跑流水线.md) |
